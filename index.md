...
The general target for this work was to add support for generating VOParquet output https://www.ivoa.net/documents/Notes/VOParquet/ from the qserv-kafka bridge.
...
We settled on 2000 as the batch size through trial and error, and based on recommendations from forum resources. Initially we went with higher numbers, but ended-up with OOM pods kills.
...
As described this leads to increased memory usage during writing, which becomes more prevelant during large queries to wide columns. Note that to handle the largest query from the benchmarks below, we had to increase the memory request limit to 500MB, otherwise the pods would OOM. This does seem however to be close to the limit of what QServ supports as well, as requesting more rows than that produced a QServ memory error (going from 200k to 300k rows).
...
Memory profiling showed the implementation successfully batched data through without retention of the full data in memory. We observed about a 15-25% memory-to-output ratio.
...
Worth noting that we also tested with a larger batch size of 5k rows, and while the memory did spike up to 450MB, it completed faster with a runtime of 180 seconds.  
...
I've also noticed that in the current implementation and on a fresh deploy, upon running the 200k query the memory usage settles to around +30 to +50 MB from the baseline it was before running the query. This could be an indication of something not being properly cleaned up, though we haven't been able to track this down. However if we repeat these large-result-set queries the baseline does seem to settle at the same point after the spikes during processing.
